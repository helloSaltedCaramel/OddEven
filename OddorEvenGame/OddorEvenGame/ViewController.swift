...
/*
 ...
 2. 사용자는 가진 구슬 개수를 걸고 홀짝 중의 하나를 선택합니다.
 3. 결과값이 화면에 보여진다.
 */
import UIKit
...
class ViewController: UIViewController {
    ...
    func getWinner(count : Int, select: String) {
        ...
        if comType == select {
            ...
            self.calculateBall(winner: "user", count: count)
        }else {
            result = result + "(Com Win)"
            print("Computer win")
            self.calculateBall(winner: "com", count: count)
        }
    ...
    func calculateBall(winner: String, count: Int) {
        if winner == "com" {
